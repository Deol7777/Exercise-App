...
**It installs to a home screen**: `src/app/manifest.ts` (standalone, portrait,
`short_name` "Exercise") plus the `appleWebApp` block in the root layout, which
...
where the frog is a smudge. `favicon.ico` carries 16/32/48/256 as separate
images so the browser draws the right one rather than downscaling a detailed
square. The frog needs two things a resize would not give it: flattening onto
an opaque square of its own olive, so iOS does not round already-rounded
corners and render the gap black, and scaling to 92% so the barbell survives
the circle Android crops a maskable icon to. The manifest's colours are the
default `rose` theme's, because a manifest is fetched once at install time
without a session.
