...
import { ExerciseEntryCard } from "@/features/training/components/exercise-entry-card";
import { FinishWorkoutDialog } from "@/features/training/components/finish-workout-dialog";
...
export function WorkoutLogger({
  ...
  /**
   * A session with no exercises on it is not a short workout, it is a session
   * that never happened — so ending an empty one deletes it rather than writing
   * a blank day into history.
   */
  const abandon = useMutation(
    crossTabMutation(
      () => apiFetch(`/api/workout-sessions/${session?.id}`, { method: "DELETE" }),
      "Could not abandon the workout session.",
    ),
  );
  ...
  const busy = start.isPending || finish.isPending || abandon.isPending || addExercise.isPending;
  ...
  /** Nothing logged yet: the only honest way out is to throw the session away. */
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <FinishWorkoutDialog
        abandon={isEmpty}
        onConfirm={() => (isEmpty ? abandon.mutate() : finish.mutate())}
        disabled={busy}
        pending={isEmpty ? abandon.isPending : finish.isPending}
      />
