...
/**
 * A segmented control — the range switch, and each card's line-or-bars toggle.
 *
 * Links, not buttons, so this stays a server component: the choice is already
 * in the URL and the server already re-renders the chart for it, which leaves
 * nothing for JavaScript to do. It also means a middle-click opens the other
 * view in a tab, and the back button undoes a switch.
 *
    ...
    >
      {options.map((option) => {
        ...
        return (
          <Link
            key={option.value}
            href={`/progress?${new URLSearchParams({ ...query, [param]: option.value })}`}
            scroll={false}
